// Renders a packed Aztec matrix the way the watchapp's code layer does, to a
...
//
// It deliberately shares aztec_matrix.h with the watchapp, so a change to the
...
#include "../watchapp/src/c/aztec_matrix.h"
...
int main(int argc, char **argv) {
  ...
  if (modules < AZTEC_MIN_MODULES || modules > AZTEC_MAX_MODULES) {
    fprintf(stderr, "module count %d outside %d..%d\n", modules,
            AZTEC_MIN_MODULES, AZTEC_MAX_MODULES);
    return 1;
  }

  uint8_t matrix[AZTEC_MAX_MATRIX_BYTES];
  FILE *in = fopen(argv[2], "rb");
  if (!in) {
    perror(argv[2]);
    return 1;
  }
  const size_t expected = aztec_matrix_bytes(modules);
  ...
  const int px = aztec_pixels_per_module(CANVAS_WIDTH, modules);
  const int size = px * modules;
  const int ox = (CANVAS_WIDTH - size) / 2;
  const int oy = AZTEC_SYMBOL_TOP;

  for (int row = 0; row < modules; row++) {
    int col = 0, run_start = 0, run_length = 0;
    while (aztec_next_run(matrix, modules, row, &col, &run_start, &run_length)) {
